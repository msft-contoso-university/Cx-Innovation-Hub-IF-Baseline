...
Simulates a project manager creating a task, editing it, assigning it to a
teammate, and finally deleting it — exercising the full task write path.
...
class TaskLifecycleUser(TaskifyBaseUser):
    """User that creates, updates, assigns and deletes a task."""

    weight = 3

    @task
    def task_lifecycle(self):
        """Create a task, edit it, assign it, then delete it."""
        ...
        try:
            ...
            with self.client.put(
                f"/api/tasks/{task_id}",
                json={
                    "title": f"{created.get('title', 'Load test task')} (edited)",
            ...
            # Assign the task to a known user (or unassign when no users loaded)
            assignee_id = None
            if self.users:
